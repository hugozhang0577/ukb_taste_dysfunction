#!/usr/bin/env Rscript
#> in   input/gwas_genotype/apoe_genotypes.raw
#> ship output/apoe_diplotype/plot_data_association_results.csv -> input/assoc_results/apoe_diplotype_association.csv
...
GENO_DIR <- file.path(PROJECT_DIR, "input", "gwas_genotype")
PHENO_FILE <- Sys.getenv("PHENO_FILE", file.path(
  PROJECT_DIR, "output", "gwas_sample_qc",
  "taste_2w_strict_white_gwas_pheno.txt"))
RES_DIR <- file.path(PROJECT_DIR, "output", "apoe_diplotype")
dir.create(RES_DIR, showWarnings = FALSE, recursive = TRUE)

# ---- genotype -> diplotype -------------------------------------------------
geno <- fread(file.path(GENO_DIR, "apoe_genotypes.raw"))
setnames(geno,
...
# Export and dx upload to RAP  (the tables below feed Figure 2b and the appendix)
