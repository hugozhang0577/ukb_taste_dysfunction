#> in   output/feature_manifest/master_feature_manifest_final.csv
#> in   output/ml_ready/group1_full.rds
#> in   output/subtyping/clusters/cluster_assignments_g1_f_k4.rds
#> in   output/subtyping/clusters/cluster_assignments_g1_m_k4.rds
#> in   output/subtyping/inputs/clustering_input_g1_f.rds
#> in   output/subtyping/inputs/clustering_input_g1_m.rds
#> in   output/subtyping/mofa/mofa_loadings_g1_f.rds
#> in   output/subtyping/mofa/mofa_loadings_g1_m.rds
#> in   output/subtyping/reports/cluster_profiles_f_k4.csv
#> in   output/subtyping/reports/cluster_profiles_m_k4.csv
#> out  output/figures/fig3a_feature_sample_heatmap.pdf
# A headless worker has no X11; fall back to cairo for the diagnostic PNGs.
...
ROW_ORDER_CSV <- "output/subtyping/reports/fig3_row_order.csv"
...
top_anno <- HeatmapAnnotation(
  Subtype = anno_block(
    ...
    labels    = c("A\nAging\nfrailty", "B\nPsycho-\nsomatic",
                  "C\nCardio-\nmetabolic", "D\nYoung\nidiopathic"),
    labels_gp = gpar(col = "white", fontface = "bold", fontsize = 6.6)
  ),
  ...
  annotation_height = unit.c(
    unit(8.2, "mm"),  # Subtype block, holds the 3-line name text
    unit(1.6, "mm"),  # Depression (PHQ-9)
    unit(1.6, "mm"),  # Sex
    unit(1.6, "mm"),  # Age band
    unit(1.6, "mm")   # BMI band
  ),
  annotation_name_side = "left",
  annotation_name_gp = gpar(fontsize = 6.8, fontface = "bold"),
  show_legend = TRUE,
  gap = unit(0.8, "mm"),
  border = TRUE,
  annotation_legend_param = list(
    "Depression (PHQ-9)" = list(title_gp = gpar(fontsize = 6.8, fontface = "bold"),
                                labels_gp = gpar(fontsize = 6.8)),
    Sex       = list(title_gp = gpar(fontsize = 6.8, fontface = "bold"),
                     labels_gp = gpar(fontsize = 6.8)),
    "Age band" = list(title_gp = gpar(fontsize = 6.8, fontface = "bold"),
                      labels_gp = gpar(fontsize = 6.8), ncol = 1),
    "BMI band" = list(title_gp = gpar(fontsize = 6.8, fontface = "bold"),
                      labels_gp = gpar(fontsize = 6.8), ncol = 1)
  )
)
...
ht <- Heatmap(
  ...
  row_title_gp        = gpar(fontsize = 6.8, fontface = "bold", col = "black"),
  row_title_rot       = 0,
  row_title_side      = "left",
  ...
  heatmap_legend_param = list(
    title       = "z-score",
    title_gp    = gpar(fontsize = 7.7, fontface = "bold"),
    labels_gp   = gpar(fontsize = 6.8),
...
# 7. Save outputs: panel a only, the feature x sample heatmap.
cat("[7/7] saving outputs (panel a)\n")
...
save_heatmap(function() cairo_pdf(file.path(OUT_DIR, "fig3a_feature_sample_heatmap.pdf"),
                                  width = 168/25.4, height = 90/25.4, family = "Arial"))
save_heatmap(function() png(file.path(OUT_DIR, "fig3a_feature_sample_heatmap.png"),
                            width = 168, height = 90, units = "mm", res = 300,
                            type = "cairo", family = "Arial"))

cat("\n=== Done ===\n")
cat("Saved to:", OUT_DIR, "\n")
cat("  fig3a_feature_sample_heatmap.{pdf,png}\n")
