...
#> out  output/mwas/effective_tests/metabolite_clusters.csv
...
cat("    numeric matrix:", nrow(nmr_mat), "x", ncol(nmr_mat), "\n")
...
cat("    compression: ", round(M / n_clusters, 1), "x\n")
  ...
  if (n_override > 0) {
    ...
    if (length(override_clusters) > 0) {
      ...
      for (k in seq_len(n_show)) {
        ...
        cat(sprintf("    %-6d  %-25s %-12s  %-25s %-12s  %.2fx\n",
                    cid,
...
cat("  Compression ratio:", round(M / n_clusters, 1), "x\n\n")
